...
1. **Temel Bileşenler Analizi (Principal Components Analysis - PCA)**: çok boyutlu verilerinin görselleştirilmesinde faydalı. Boyut küçültme (dimension reduction) ve gözetimli öğrenme yöntemlerinin öncesinde verileri işlemden geçirme amacıyla kullanılabilir (örneğin PCA regresyonu). 
...
Normalize edilmesinden kasıt $\phi$ ağırlıklarının (*factor loadings*) toplamının 1 olmasıdır: 
...
Her bir gözlem sadece bir grupta yer alır, yani $C_{1} \cup C_{2} \cup \ldots \cup C_{K}=\{1, \ldots, n\}$

Ayrıca her gözlem sadece bir grupta yer alabilir: $C_{k} \cap C_{k^{\prime}}=\emptyset$, her $k\neq k^{\prime}$ için. 
...
1. Gözlemleri tesadüfi bir şekilde $K$ rgruptan birine ata. 
...
* Birbirine yakın grupları da birlikte kümelendir. 
]
